...
satellite_network_routes_dir = os.path.join(satellite_network_dir, satellite_network_dir, ) \
                               + '_isls_plus_grid_twostation_algorithm_free_one_only_over_isls/dynamic_state_{}ms_for_{}s'.format(time_step_ms, simulation_end_time_s)
...
#####################################################################
        ...
        for i in range(1, len(satellites) + len(ground_stations) + 1):
            sat_switch_name = 's{}'.format(i)
            info('add all switches ')
            sat_net_switches_all_with_only_gsls[i] = self.addSwitch(sat_switch_name)
        ...
        for (a, b) in isl_list:
            ...
            # Add isl link
            self.addLink(sat_net_switches_all_with_only_gsls[a + 1], sat_net_switches_all_with_only_gsls[b + 1],
                         bw=isl_data_rate_megabit_per_s, delay=str(delay)+'ms', max_queue_size=isl_max_queue_size_pkts)
        ...
        for time_since_epoch_ms in range(0, simulation_end_time_s, time_step_ms):
            ...
            for ground_station in ground_stations:
                ...
                for sid in range(len(satellites)):
                    ...
                    if distance_m <= MAX_GSL_LENGTH_M:
                        ...
                        # print("\ngs2sat---->  ", delay)
                        self.graphs_sat_net_graph_all_with_only_gsls.add_edge(sid, len(satellites) + ground_station["gid"], weight=delay)
            ...
            # add GSL link
            # ground_fstate_list = cal_access_sats_each_time_step(ground_station_satellites_in_range)
            # for index, gs_dpid in enumerate(ground_dpid_list):
            #
            #     gsl_distance_m = distance_m_ground_station_to_satellite(
            #                         ground_stations[index],
            #                         satellites[ground_fstate_list[index]],
            #                         str(epoch),
            #                         str(time)
            #                     )
            #     gsl_delay = round(gsl_distance_m / light_speed, 4) * 1000
            #
            #     self.addLink(sat_net_switches_all_with_only_gsls[gs_dpid],
            #                  sat_net_switches_all_with_only_gsls[ground_fstate_list[index] + 1],
            #                  bw=gsl_data_rate_megabit_per_s, delay=str(gsl_delay)+'ms', max_queue_size=gsl_max_queue_size_pkts)
        ...
        for i in range(1, len(ground_stations) + 1):
            ...
            Hosts_On_Ground[i] = self.addHost(gs_switch_name, ip='10.0.{}.{}'.format((len(satellites) + i)//255, (i + len(satellites)) % 255))
        ...
        while current_time < self.simulation_end_time_s*1000:
            ...
            for sat in switch2satellites:
                for intf in sat.intfList():
                    if intf.link and type(intf.link.intf1.node) == type(intf.link.intf2.node): # the link is between switch and switch

                        src_2_dst_node_ids = (int(intf.link.intf1.node.dpid, 16)-1, int(intf.link.intf2.node.dpid, 16)-1) # 卫星编号从0开始， 交换机编号从1开始
                        src_2_dst_node_ids_reverse = (int(intf.link.intf2.node.dpid, 16)-1, int(intf.link.intf1.node.dpid, 16)-1) # 卫星编号从0开始， 交换机编号从1开始
                        ...
                        if src_2_dst_node_ids not in links_sat:
                            ...
                            if src_2_dst_node_ids[0] < len(satellites) and src_2_dst_node_ids[1] < len(satellites):
                                distance_m = distance_m_between_satellites(satellites[src_2_dst_node_ids[0]], satellites[src_2_dst_node_ids[1]], str(epoch), str(simulation_time))
